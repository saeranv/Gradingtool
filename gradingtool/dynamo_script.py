...
def main(p, topo_pts, topo_crv, bldg_crv, anaconda_path, update_tool = False):
    ...
    if not os.path.isdir(anaconda_path):
        ...
        anaconda_path = "C:\\Users\\{}\\AppData\\Local\\Continuum\\Anaconda2\\".format(user_name)
    
    if not os.path.isdir(anaconda_path):
        where_path = "Cannot find path to Anaconda at {}.".format(anaconda_path)
        raise Exception( where_path +
                        " It is usually found in C:\Users\%USERNAME%\AppData\Local\Continuum\Anaconda2. "
